...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        ### PUT YOUR CODE HERE ###
        states, actions = self.state_dim, self.action_dim
        terminal = states-1
        while True:
            delta=0
            v=V
            ...
            for state in range(states):
                ...
                for action in range(actions):
                    for newState in range(states):
                        Q[action]+=self.mdp.P[state, action, newState]*(self.mdp.R[state, action, newState]+self.gamma*v[newState])

                V[state]= np.max(Q)
                delta= max(delta, abs(v[state]-V[state]))
            if delta<theta:
                break
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while True:
            ...
            for state in range(states):
                for action in range(actions):
                    for newState in range(states):
                        V[state]+= policy[state, action] * self.mdp.P[state, action, newState]*(self.mdp.R[state,action,newState]+self.gamma * v[newState])
