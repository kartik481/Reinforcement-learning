    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(*args, **kwargs):
            ### PUT YOUR CODE HERE ###
            return max(self.epsilon_min, self.epsilon_start - ((self.epsilon_start-self.epsilon_min) * (timestep /(self.exploration_fraction * max_timestep))))
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        :return (sample from self.action_space): action the agent should perform
            """
        ### PUT YOUR CODE HERE ###
        obses = torch.tensor(obs, dtype=torch.float)
        actions = self.critics_net.forward(obses).detach().numpy()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        ## Getting the transition from batch
        (currState, action, nextState, r_j, done) = batch
        Q_max = self.critics_target.forward(nextState).max(dim=1)[0].detach().unsqueeze(axis=1)
        Q_0 = self.critics_net.forward(currState).gather(dim = 1, index = action.long())
        Y = r_j + (1-done) * self.gamma * Q_max
        q_loss = loss(Y, Q_0)
        self.critics_optim.zero_grad()
        q_loss.backward()
        self.critics_optim.step()
        ## Perfomring a hard update
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        ### PUT YOUR CODE HERE ###
        obses = torch.tensor(obs, dtype = torch.float)
        action_prob = self.policy.forward(obses)
        prob_dist = Categorical(action_prob)
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        for t in range(T-1,-1,-1):
            ...
            policy = self.policy.forward(torch.tensor(observations[t]).float())
            log_policy = torch.log(policy[actions[t]])
            p_loss = p_loss - G * log_policy
        p_loss/= T
